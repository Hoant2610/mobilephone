...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE product SET total_sold = total_sold - ?2 WHERE id = ?1", nativeQuery = true)
    void minusProductTotalSold(String productId,int quantity);

    //Cộng một sản phẩm đã bán
    @Transactional
    @Modifying
    @Query(nativeQuery = true, value = "Update product set total_sold = total_sold + ?2 where id = ?1")
    void plusProductTotalSold(String productId,int quantity);
